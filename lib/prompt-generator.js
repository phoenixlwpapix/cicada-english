...
Finally generate a short, vivid ImagePrompt suitable for AI image generation, in a vibrant cartoon style, capturing a key, fantastical scene from the story to spark visual imagination. Do not include any character name.
...
// 生成自定义文本的提示词
function generateCustomPrompt(text, level) {
  // 验证输入级别
  const config = levelConfig[level] || levelConfig["A2"];
...
Your task is to:
1. Analyze the text.
2. If the text is NOT in English, translate it into English suitable for CEFR level ${level}.
3. If the text IS in English, adapt it to be suitable for CEFR level ${level} (vocabulary, sentence structure).
4. Ensure the final story is around ${config.wordCount} words.
5. Include a title in H2 MarkDown format (starting with ##).
6. Generate ${config.numQuestions} multiple-choice questions based on the story.
7. Generate a short, vivid ImagePrompt suitable for AI image generation, in a vibrant cartoon style, capturing a key scene from the story. Do not include any character name.

Format the output EXACTLY as follows:

## [Story Title]
[Insert adapted/translated story here]
