...
export default function HomePage() {
  ...
  setLoading(true);
  setScore(null);
  console.log('[Frontend] Starting generation with words:', words);
  ...
  try {
    ...
    if (questionsBlock) {
      ...
      let currentQuestion = "", currentOpts = [];
      ...
      for (let line of questionLines) {
        if (/^\d+\.\s/.test(line)) {
          ...
          const correctIndex = { A: 0, B: 1, C: 2 }[correctLetter.toUpperCase()];
    ...
    if (parsedQuestions.length !== parsedAnswers.length) {
      console.error('[Frontend] Mismatch between questions and answers count');
...
const handleRegenerate = () => {
  handleGenerate(); // 重新生成内容
  setCurrentQuestion(0); // 重置题目位置
  setScore(null); // 清除分数
};
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-16 animate-fade-in">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-100 to-cyan-100 dark:from-emerald-900/50 dark:to-cyan-900/50 rounded-full mb-6">
            ...
            <span className="text-sm font-medium text-emerald-700 dark:text-emerald-300">智能英语阅读训练</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-slate-800 dark:text-slate-100 mb-6">
            个性化英语学习体验
          </h2>
          <p className="text-lg text-slate-600 dark:text-slate-300 max-w-2xl mx-auto leading-relaxed">
            输入你想学习的单词，AI将为你生成有趣的故事和阅读理解题，让学习更加生动有趣！
          </p>
        </div>
        ...
        <section className="mb-16">
          ...
          <CardContent className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">单词输入</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400">输入你想练习的英语单词</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Button 
                  ...
                  disabled={loading} 
                  className="flex-1 bg-gradient-to-r from-emerald-500 to-cyan-500 hover:from-emerald-600 hover:to-cyan-600 text-white font-medium py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 disabled:hover:scale-100"
                >
                  {loading ? (
                    <>
        ...
        {story && (
          <section className="mb-16">
            <Card ref={storyRef} className="backdrop-blur-lg bg-white/70 dark:bg-slate-800/70 border border-white/20 dark:border-slate-700/50 shadow-xl hover:shadow-2xl transition-all duration-300 group animate-fade-in-up">
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">阅读文章</h2>
                  <p className="text-sm text-slate-600 dark:text-slate-400">仔细阅读下面的故事</p>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-700 dark:to-slate-600 rounded-xl p-6 border border-slate-200/50 dark:border-slate-600/50">
                <p className="text-slate-800 dark:text-slate-100 whitespace-pre-line leading-relaxed text-base">{story}</p>
              </div>
            </CardContent>
            </Card>
          </section>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">阅读理解题</h2>
                    <p className="text-sm text-slate-600 dark:text-slate-400">测试你的理解能力</p>
                  </div>
                </div>
              ...
              <div className="bg-gradient-to-br from-slate-50 to-purple-50 dark:from-slate-700 dark:to-purple-900/20 rounded-xl p-6 border border-slate-200/50 dark:border-slate-600/50">
                <p className="text-lg font-medium text-slate-800 dark:text-slate-100 mb-6">{questions[currentQuestion]}</p>
              <div className="space-y-3">
                {options[currentQuestion].map((opt, i) => {
                  ...
                  let optionStyle = "bg-white dark:bg-slate-600 border-slate-200 dark:border-slate-500 text-slate-800 dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-500";
                  
                  if (score !== null) {
                    if (isCorrect) {
                      optionStyle = "bg-green-100 dark:bg-green-900/30 border-green-300 dark:border-green-600 text-green-700 dark:text-green-300";
                    } else if (isSelected && !isCorrect) {
                      optionStyle = "bg-red-100 dark:bg-red-900/30 border-red-300 dark:border-red-600 text-red-700 dark:text-red-300";
                    }
                  } else if (isSelected) {
                    optionStyle = "bg-purple-100 dark:bg-purple-900/30 border-purple-300 dark:border-purple-600 text-purple-700 dark:text-purple-300";
                  }

                  return (
                    <label key={i} className={`block cursor-pointer transition-all duration-200 ${score !== null ? 'cursor-not-allowed' : 'hover:scale-[1.02]'}`}>
                      <div className={`flex items-center space-x-4 p-4 rounded-xl border-2 transition-all duration-200 ${optionStyle}`}>
              ...
              <div className="border-t border-slate-200 dark:border-slate-600 pt-6">
                {currentQuestion === questions.length - 1 && score === null && (
                  <Button 
                ...
                {score !== null && (
                  <div className="space-y-4">
                    ...
                    <Button
                      ...
                      className="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-medium py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
                    >
                      <Sparkles className="w-5 h-5 mr-2" />
                      生成新的学习内容
                    </Button>
